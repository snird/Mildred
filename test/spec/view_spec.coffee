describe 'View', ->
  ...
  setModel = ->
    model = new Mildred.Model foo: 'foo', bar: 'bar'
    view.model = model
  ...
  it 'should not attach itself if autoAttach is false', ->
    class NoAutoAttachView1 extends Mildred.View
      autoAttach: false
      autoRender: true
      container: testbed
  ...
  it 'should return proper template data for Chaplin collections', ->
    model1 = new Mildred.Model foo: 'foo'
    model2 = new Mildred.Model bar: 'bar'
  ...
  describe 'Events', ->
    ...
    class EventedViewParent extends Mildred.View
      listen:
      # self
        'ns:a': 'a1Handler'
        'ns:b': ->
    ...
    it 'should bind to own events declaratively', ->
      view = new EventedView model: new Mildred.Model()
    ...
    it 'should throw an error when corresponding method doesn’t exist', ->
      class ErrorView extends Mildred.View
        listen:
          'stuff': 'stuff'
  ...
  describe 'Disposal', ->
    ...
    it 'should not dispose itself when the collection model is disposed', ->
      collection = new Mildred.Collection [{a: 1}, {a: 2}, {a: 3}]
      view = new TestView {collection}
