describe 'Router and Route', ->
  ...
  beforeEach ->
    router = new Mildred.Router randomOption: 'foo', pushState: false
  ...
  describe 'Creating Routes', ->
    ...
    it 'should have a match method which returns a route', ->
      ...
      expect(route).to.be.a Mildred.Route
  ...
  describe 'Routing', ->
    ...
    it 'should match correctly when using the root option', ->
      subdirRooter = new Mildred.Router randomOption: 'foo', pushState: false, root: '/subdir/'
      spy = sinon.spy()
      Backbone.on 'router:match', spy
  ...
  describe 'Route Matching', ->
    ...
    it 'should compare route value', ->
      route = new Mildred.Route 'params', 'hello', 'world'
  ...
  describe 'Route Reversal', ->
    ...
    it 'should reject reversals when there are not enough params', ->
      route = new Mildred.Route 'params/:one/:two', 'null', 'null'
  ...
  describe 'Query string extraction', ->
    ...
    it 'should extract query string parameters from an url', ->
      ...
      query = Mildred.utils.queryParams.stringify input
  ...
  describe 'Setting the router:route handler', ->
    ...
    it 'should route when receiving a path', ->
      ...
      Backbone.trigger 'router:route', url: path, options
      expect(passedRoute).to.be.an 'object'
      ...
      expect(->
        Backbone.trigger 'router:route', 'different-path', options
      ).to.throwError()
    ...
    it 'should route when receiving a name', ->
      ...
      Backbone.trigger 'router:route', name: 'home'
    ...
    it 'should route when receiving both name and params', ->
      ...
      params = id: '123'
      Backbone.trigger 'router:route', 'phonebook', params
    ...
    it 'should route when receiving controller and action name', ->
      ...
      Backbone.trigger 'router:route', controller: 'home', action: 'index'
    ...
    it 'should route when receiving controller and action name and params', ->
      ...
      params = id: '123'
      Backbone.trigger 'router:route', controller: 'phonebook', action: 'dial', params
      expect(passedRoute.controller).to.be 'phonebook'
      expect(passedRoute.action).to.be 'dial'
      expect(passedRoute.path).to.be "phone/#{params.id}"
      expect(passedParams).not.to.be params
    ...
    it 'should pass options and call the callback', ->
      ...
      params = id: '123'
      options = replace: true
      Backbone.trigger 'router:route', 'phonebook', params, options
    ...
    it 'should throw an error when no match was found', ->
      expect(->
        Backbone.trigger 'router:route', 'phonebook'
      ).to.throwError()
  ...
  describe 'Extendability', ->
    ...
    it 'should be extendable', ->
      expect(Mildred.Router.extend).to.be.a 'function'
      expect(Mildred.Route.extend).to.be.a 'function'

      DerivedRouter = Mildred.Router.extend()
      derivedRouter = new DerivedRouter()
      expect(derivedRouter).to.be.a Mildred.Router

      DerivedRoute = Mildred.Route.extend()
      derivedRoute = new DerivedRoute 'foo', 'foo#bar'
      expect(derivedRoute).to.be.a Mildred.Route
