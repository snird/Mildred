class Mildred.Route
  ...
  handler: (pathParams, options) =>
    ...
    if typeof pathParams is 'object'
      query = Mildred.utils.queryParams.stringify options.query
      params = pathParams
      path = @reverse params
    else
      ...
      if not query?
        ...
        options.query = Mildred.utils.queryParams.parse query
    ...
    Backbone.trigger 'router:match', route, actionParams, options
